...
<<finalcode,echo=FALSE,results='hide',message=FALSE,warning=FALSE,error=FALSE,fig.show='hide'>>=
setwd('~/Git/Bayesian_Portfolio_Paper/Paper/')
source('../Code/1-final-key.R')
@
...
\begin{flalign}
    \mu_p = \textrm{E}[p] = \textrm{E}[\bm{\omega}^T \bm{X}] = \bm{\omega}^T \bm{\mu}
\end{flalign}
...
\begin{flalign}
    \bm{\Sigma} = \textrm{E}\left[ (\bm{X} - \textrm{E}[\bm{X}]) (\bm{X} - \textrm{E}[\bm{X}])^T \right]
\end{flalign}
...
\begin{flalign}
    \bm{\omega}^* = \frac{\bm{\Sigma}^{-1} \bm{1}}{\bm{1}^T \bm{\Sigma}^{-1} \bm{1}}
\end{flalign}
...
\begin{flalign}
    L(\omega, \bm{\lambda}) = \bm{\omega}^T \bm{\Sigma} \bm{\omega} - \bm{\lambda} \left( \bm{A} \bm{\omega} - \bm{b} \right)
\end{flalign}
...
<<frontier, echo=FALSE,fig.width=5, fig.height=2.75, fig.keep='last', message=FALSE, fig.align='center', fig.cap='Efficient portfolio frontier and optimal allocation point, using assets described below and the classical method of portfolio optimization.'>>=
# Make Naive Port
print(p.eff.naive)
@
...
\begin{flalign}
    \bm{y}|\bm{\mu},\bm{\Sigma} \sim N(\bm{\mu},\bm{\Sigma})
\end{flalign}
...
\begin{flalign}
    p(\bm{y}|\bm{\mu},\bm{\Sigma}) \propto |\bm{\Sigma}|^{-1/2} \exp \left(- \frac{1}{2} \left( \bm{y} - \bm{\mu} \right)^T \bm{\Sigma}^{-1} \left( \bm{y} - \bm{\mu} \right)  \right)
\end{flalign}

For $n$ independently and identically distributed (i.i.d.) observations, the likelihood is 

\begin{flalign}
    p(\bm{y}_1, \dots, \bm{y}_n |\bm{\mu},\bm{\Sigma}) \propto |\bm{\Sigma}|^{-n/2} \exp \left(- \frac{1}{2} \textrm{tr} \left( \bm{\Sigma}^{-1} \bm{S}_0 \right)  \right)    
\end{flalign}

$\bm{S}_0$ is the sum of squares matrix relative to $\bm{\mu}$ and is defined thusly,

\begin{flalign}
    \bm{S}_0 = \sum_{i=1}^{n}(\bm{y}_i - \bm{\mu})(\bm{y}_i - \bm{\mu})^T
    \label{}
\end{flalign}
...
\begin{flalign}
    p(\bm{\mu}| \bm{y} \bm{\Sigma}) & \propto \exp \left( -\frac{1}{2} \left( \bm{\mu} - \bm{\mu}_n \right)^T \bm{\Lambda}_n^{-1} \left( \bm{\mu} - \bm{\mu}_n \right) \right) \notag \\
...
\begin{flalign}
    \bm{\mu}_n & = \left( \bm{\Lambda}_0^{-1} + n \bm{\Sigma}^{-1} \right)^{-1} \left( \bm{\Lambda}_0^{-1} \bm{\mu}_0 + n \bm{\Sigma}^{-1} \bar{\bm{y}} \right) \notag \\
...
\begin{flalign}
    \mu^{(1)}|\bm{\mu}^{(-1)}, \bm{y} \sim N\left( \bm{\mu}_n^{(1)} + \bm{\beta}^{1|2}\left( \bm{\mu}^{(-1)} - \bm{\mu}_n^{(-1)} \right), \bm{\Lambda}^{1|2} \right)
    \label{}
\end{flalign}
...
\begin{flalign}
    \bm{\mu}_n & = \frac{\kappa_0}{\kappa_0 + n} \bm{\mu}_0 + \frac{n}{k_0 + n} \bar{\bm{y}} \notag \\
    \kappa_n & = \kappa_0 + n \notag \\
    v_n & = v_0 + n \notag \\
    \bm{\Lambda}_n & = \bm{\Lambda}_0 + \bm{S} + \frac{\kappa_0 n}{\kappa_0 + n} \left(\bar{\bm{y}}  -\mu_0  \right) \left( \bar{\bm{y}} -\mu_0 \right)^T
\end{flalign}
...
\begin{flalign}
  \bm{C} & = \frac{1}{\mu_p} \mathrm{D}(\bm{\omega_p}) \bm{X}^T \bm{1}_n
\end{flalign}
...
<<sixmonth, echo=FALSE,fig.width=5, fig.height=4, fig.keep='last', message=FALSE, fig.pos='H', fig.cap="Indexed stock returns for the period of 07-18-2011 to 12-27-2011.">>=
print(p.returns)
@
...
t1 = xtable(stocks.table, label = 'tab:returns', digits = 2, caption = 'Historical average returns, standard deviations and Sharpe ratios for the candidate assets over previous six months.')
print(t1, scalebox=0.85)
@
...
t2 = xtable(compare, align= 'lrrr', label = 'tab:results', digits = 2, caption = 'Annualized portfolio return and risk profiles for each of the estimation methods considered. The final three portfolios use Bayesian estimates.')
print(t2)
@

<<t3, echo=FALSE,results='asis'>>=
t3 = xtable(final.weights, align = 'lrrrrrrrr', label = 'tab:weights', digits = 2, caption = 'Asset allocations for each of the portfolios in this project. The final three portfolios use Bayesian methods.')
print(t3, scalebox=0.85)
@
...
<<cumret, echo=FALSE,fig.width=5, fig.height=4, fig.keep='last', message=FALSE, fig.cap='Cumulative returns for each model portfolio, as tested against asset data from 01-09-2012 to 12-30-2013.', fig.pos='t'>>=
print(p.cumret)
@
...
<<t4, echo=FALSE,results='asis'>>=
t4 = xtable(final.weights, align = 'lrrrrrrrr', label = 'tab:test', digits = 2, caption = 'Expected returns, standard deviations and Sharpe ratios for the candidate assets for the various portfolios over the test period. The data used for estimating these statistics cover 01-09-2012 to 12-30-2013.')
print(t4, scalebox=0.85)
@
...
<<probsdist, echo=FALSE, fig.width=5, fig.height=3, fig.keep = 'hold', out.width='\\linewidth', fig.cap='A joint density plot for the market index and the Bayesian portfolio with an informative prior. Contour lines and annotation for where $Y > X$ are included.'>>=
print(p.ipdist)
@

<<probsdens, echo=FALSE, fig.width=5, fig.height=3, fig.keep = 'hold', out.width='\\linewidth', fig.cap='An overlay of the two densities for the market index and the Bayesian portfolio with an informative prior.'>>=
print(p.ipdens)
@
...
<<t6, echo=FALSE,results='asis'>>=
t6 = xtable(t(cont.means), align = 'lrrrrrrrr', label = 'tab:contmeans', digits = 2, caption = 'Mean asset contributions to 5,000 simulated portfolio returns for the five portfolios examined in this project. The final three rows refer to portfolios constructed using Bayesian estimates of the parameters.')
print(t6, scalebox = .85)
@

<<t7, echo=FALSE,results='asis'>>=
t7 = xtable(t(cont.sds), align = 'lrrrrrrrr', label = 'tab:contsds', digits = 2, caption = 'Standard deviation of asset contributions to 5,000 simulated portfolio returns for the five portfolios examined in this project. The final three rows refer to portfolios constructed using Bayesian estimates of the parameters.')
print(t7, scalebox = .85)
@
...
\begin{thebibliography}{99}
        ...
        \bibitem{black92} Black, F., \& Litterman, R. (1992). Global portfolio optimization. Financial \textit{Analysts Journal}, 48(5), 28-43.
        ...
        \bibitem{geman84} Geman, S., \& Geman, D. (1984). Stochastic relaxation, Gibbs distributions, and the Bayesian restoration of images.\textit{ Pattern Analysis and Machine Intelligence, IEEE Transactions on}, 6, 721-741.
        \bibitem{hoff09} Hoff, P. D. (2009). \textit{A first course in Bayesian statistical methods}. Springer Science \& Business Media.
        \bibitem{kruschke10} Kruschke, J. (2010). \emph{Doing Bayesian data analysis: A tutorial introduction with R}. Academic Press.
    ...
    \bibitem{rachev04} Ortobelli, S., Rachev, S.T. \& Schwartz, E.  (2002). The problem of optimal asset allocation with stable distributed returns. \textit{Stochastic Processes and Functional Analysis: Recent Advances, in Honor of Prof. M.M. Rao}, Marcel Dekker Inc., 238, 295-347.
