...
\begin{flalign}
    \mu^{(1)}|\bm{\mu}^{(-1)}, \bm{y} \sim N\left( \mu_n^{(1)} + \beta^{1|2}\left( \bm{\mu}^{(-1)} - \bm{\mu}_n^{(-1)} \right), \bm{\Lambda}^{1|2} \right)
    \label{}
\end{flalign}
...
\begin{flalign}
    \bm{\mu}_n & = \frac{\kappa_0}{\kappa_0 + n} \mu_0 + \frac{n}{k_0 + n} \bar{y} \notag \\
    \kappa_n & = \kappa_0 + n \notag \\
    v_n & = v_0 + n \notag \\
    \bm{\Lambda}_n & = \bm{\Lambda}_0 + \bm{S} + \frac{\kappa_0 n}{\kappa_0 + n} \left(\bar{y}  -\mu_0  \right) \left( \bar{y} -\mu_0 \right)^T
\end{flalign}
